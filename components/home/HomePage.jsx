...
export default function HomePage({ events, schedule, videos }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const localStorageState = JSON.parse(window.localStorage.getItem('won-rva'));
        if (localStorageState && typeof localStorageState === 'object' && localStorageState.locale) {
  ...
  return (
    <div id="home" className="Home">
      <nav className="fixed top-0 left-0 right-0 bg-black/25 z-50 h-16" role="navigation">
      ...
      <Introduction header={header} intro={intro} locale={locale} changeState={changeState} />
